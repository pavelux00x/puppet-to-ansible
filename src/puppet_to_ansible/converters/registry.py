...
class ConverterRegistry:
    ...
    def auto_discover(self) -> None:
        """Import all modules in puppet_to_ansible.converters and register BaseConverter subclasses."""
        import puppet_to_ansible.converters as pkg
        for _finder, module_name, _ispkg in pkgutil.iter_modules(pkg.__path__):
            if module_name in ("base", "registry"):
                continue
            try:
                mod = importlib.import_module(f"puppet_to_ansible.converters.{module_name}")
