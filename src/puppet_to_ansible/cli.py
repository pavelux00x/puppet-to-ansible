...
from puppet_to_ansible.converters.manifest_converter import ConversionResult, ManifestConverter
from puppet_to_ansible.generators.playbook import InventoryGenerator, PlaybookGenerator, RoleGenerator
from puppet_to_ansible.parser.parser import ParseError, parse_file
from puppet_to_ansible.parser.preprocessor import ManifestPreprocessor
...
from puppet_to_ansible.utils.hiera_resolver import HieraResolver, build_hiera_resolver
...
def convert_all_cmd(
    ...
    if hieradata_dir and not dry_run:
        ...
        from puppet_to_ansible.templates.hiera_to_vars import HieraConverter
        hc = HieraConverter()
...
def analyze_puppetfile_cmd(
    ...
    _setup_logging(verbose)
    from puppet_to_ansible.puppetfile.parser import PuppetfileParser
    from puppet_to_ansible.puppetfile.mapper import PuppetfileMapper, STATUS_MAPPED, STATUS_BUILTIN, STATUS_CONVERTED, STATUS_GIT, STATUS_LOCAL, STATUS_MANUAL, STATUS_UNKNOWN
...
def _write_puppetfile_report(report, path: Path) -> None:
    ...
    from puppet_to_ansible.puppetfile.mapper import STATUS_MAPPED, STATUS_BUILTIN, STATUS_CONVERTED, STATUS_GIT, STATUS_LOCAL, STATUS_MANUAL, STATUS_UNKNOWN
    lines = [
